...
function Session:command(name, add_position)
  ...
  if add_position then
    ...
    filepos = " "..path..';'..(dirty or path)..':'..
              tostring(sel.col)..':'..tostring(sel.line)
  end
  self.write_fd:write(name..filepos.."\n")
...
function Session:cycle()
  ...
  -- event with filepath
  local result = parse_answer(self.read_fd:read("*l"))
  if result ~= nil then
    vis.events.emit("NISGOTANSWER", self.file, result)
  end
end
